...
class AdaptiveBoundaryCCM(nn.Module):
    ...
    def _init_weights(self):
        ...
        nn.init.normal_(self.boundary_head[-1].weight, std=0.01)
        nn.init.constant_(self.boundary_head[-1].bias[0], 3.0)  # log(b1) ≈ 3 → b1 ≈ 20
        # 【优化】初始间隔设大一点，防止初始阶段就触碰下限
        nn.init.constant_(self.boundary_head[-1].bias[1], 1.0)  # delta12 (approx 2.7x)
        nn.init.constant_(self.boundary_head[-1].bias[2], 1.0)  # delta23
    ...
    def forward(self, feature_map, spatial_shapes=None, real_counts=None):
        ...
        # 限制 log 值范围，防止数值溢出
        # 1.0 <= log_b1 <= 8.0 (2.7 ~ 2980)
        log_b1 = raw_out[:, 0].clamp(min=1.0, max=8.0)

        # 【关键修复】提高最小间隔阈值，以满足 box_ops.py 中 1.5倍 的验证要求
        # ln(1.5) ≈ 0.4055，所以设置 min_log_gap = 0.42 是安全的
        min_log_gap = 0.42
...
class TrueAdaptiveBoundaryLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # 2. Spacing Loss - 物理约束（防止边界过小或过大）
        # 增加对 b3 的约束，防止其超过 10000 (log(10000) ≈ 9.21)
        loss_spacing = (
                F.relu(0.4 - log_b[:, 0]) +  # log(b1) >= 0.4 (b1 >= ~1.5)
                F.relu(log_b[:, 2] - 9.5)  # log(b3) <= 9.5
        ).mean()
