...
# 添加一个便捷的函数包装器，用于直接调用
def ms_deform_attn(value, value_spatial_shapes, value_level_start_index, sampling_locations, attention_weights,
                   im2col_step=64):
    """
    多尺度可变形注意力机制的主函数
    """
...
class MultiScaleDeformableAttention(torch.nn.Module):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__()
        # 这里可以添加一些初始化参数，但为了简化，我们暂时忽略

    def forward(self, value, value_spatial_shapes, value_level_start_index, sampling_locations, attention_weights):
